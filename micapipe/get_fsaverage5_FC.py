...
Generates multiple functional connectomes based on micapipe's provided parcellations.
    ...
    parcFil     :  str
                    Annotation file name (to be prepended by 'lh' or 'rh')
    parcNam     :  str
                    Parcellation name
    performNSR  :  int
...
Adapted by Chris Norman (Oct 2022)
...
parcFil = sys.argv[4]
parcNam = sys.argv[5]
performNSR = 1 # Default: TRUE
performGSR = 1 # Default: TRUE
func_lab = '_space-rsfmri_desc-singleecho'
...
# ------------------------------------------
# fsaverage5 processing
# ------------------------------------------

# Find and load surface-registered cortical timeseries
# os.chdir(funcDir+'/surfaces/')
x_lh = " ".join(glob.glob(funcDir+'/surfaces/'+'*space-fsaverage5_lh_10mm*'))
x_rh = " ".join(glob.glob(funcDir+'/surfaces/'+'*space-fsaverage5_rh_10mm*'))
...
# fsaverage5
data_corr = get_regressed_data(x_spike, data, performNSR, performGSR, 'fsaverage5')

# save spike regressed and concatenanted timeseries (subcortex, cerebellum, cortex)
np.savetxt(funcDir+'/surfaces/' + subject + '_rsfmri_space-fsaverage5_desc-timeseries_clean.txt', data_corr, fmt='%.6f')

# Load parcellation labels
labels_L = nib.freesurfer.io.read_annot(parcDir+'/lh.'+parcFil)[0]
labels_R = nib.freesurfer.io.read_annot(parcDir+'/rh.'+parcFil)[0]

# check if lh and rh encode the same values
if np.max(labels_L) == np.max(labels_R):
    # Note: Only non-zero labels are added to
    labels_R[labels_R>0] = labels_R[labels_R>0] + labels_L.max()

# Concatenate parcellation
thisparc = np.concatenate((labels_L,labels_R))
...
if np.isnan(exclude_labels[0]) == False:
    ...
    ts_r = np.triu(ts_r)
np.savetxt(funcDir + '/surfaces/' + subject + '_rsfmri_space-fsaverage5_atlas-' + parcNam + '_desc-timeseries.txt', ts, fmt='%.12f')
np.savetxt(funcDir + '/surfaces/' + subject + '_rsfmri_space-fsaverage5_atlas-' + parcNam + '_desc-FC.txt', ts_r, fmt='%.6f')
